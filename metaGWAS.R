metaGWAS <- function(
  ...
  ####################
  setwd(wdir)
  if (exists(projname)) { dir_prefix_name <- projname } else { dir_prefix_name <- "./" }
  pop <- projname
  if (exists(method)) {gwas_method <- method} else {gwas_method <- "MLM"}
  if (exists(LOCO)) { LOCO <- LOCO } else { LOCO <- FALSE }
  if (exists(maf)) { maf <- maf } else { maf <- 0.02 }
  if (exists(genofile_2x)) {file_ploidy_2 <- genofile_2x} else {file_ploidy_2 <- NULL}
  if (exists(genofile_4x)) {file_ploidy_4 <- genofile_4x} else {file_ploidy_4 <- NULL}
  if (exists(genofile_6x)) {file_ploidy_6 <- genofile_6x} else {file_ploidy_6 <- NULL}
  if (exists(genofile_8x)) {file_ploidy_8 <- genofile_8x} else {file_ploidy_8 <- NULL}
  if (exists(phenofile)) {phenotype_data <- phenofile
  } else {
    print(cat("Phenotype data not provided\n",
              "if metagenome/microbiome data is provided\n", "GWAS will use taxa abundance as phenotype"))
    phenotype_data <- NULL
  }
  if (!exists(covariate_pheno)) {covariatename <- covariate_pheno} else {covariatename <- NULL}
  drop_zero_trait_value <- FALSE
  if (exists(covariate_metag_taxa)) { covariate <- covariate_metag_taxa } else { covariate <- FALSE }
  if (exists(covariate_metag_taxa)) { corr_coeff <- "full" } else { corr_coeff <- NULL }
  if (exists(pheno_taxa_strain)) { taxa_strain <- pheno_taxa_strain } else { taxa_strain <- NULL }
  if (exists(pheno_taxa_sspecies)) { taxa_species <- pheno_taxa_sspecies } else { taxa_species <- NULL }
  if (exists(metag_data_strains)) { metagenome_data_strains <- metag_data_strains } else { metagenome_data_strains <- NULL }
  if (exists(metag_data_species)) { metagenome_data_species <- metag_data_species } else { metagenome_data_species <- NULL }
  if (exists(model_effect)) { 
  ...
  if (exists(fdr)) { threshold_FDR <- fdr } else { threshold_FDR <- TRUE }
  if (exists(bonferroni)) { threshold_Bonferroni <- bonferroni } else { threshold_Bonferroni <- TRUE }
  if (exists(suggestive)) { threshold_suggestive <- suggestive } else { threshold_suggestive <- "5"}
  if (exists(perm)) { permutations <- perm } else { permutations <- "1" }
  if (exists(perm)) { cores <- cores } else { cores <- 1 }
